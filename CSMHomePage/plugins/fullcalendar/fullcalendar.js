...
var View = fc.View = Class.extend({
	...
	function renderView(delta, viewType) {
        ...
        //hack to communicate between fullCalendar control and datepicker. This sets datepicker month to match fullCalendar month..... 
        if (delta > 0) {
            var cm = new Date(currentView.end._d.getFullYear(), currentView.end._d.getMonth(), currentView.end._d.getDate() - 10);
            var currentMonth = cm.getMonth() + 2;
            var currentYear = currentView.end._d.getFullYear();
          //  console.log(currentMonth);
            //if ($("#startDate").val() != "") {
            //    var startDate = new Date($('#startDate').val());
            //    $("#startDate").datepicker('setDate', startDate.getFullYear().toString() + '/' + startDate.getMonth().toString() + '/' + startDate.getDate().toString());
            //} else {
            //    $("#startDate").datepicker('setDate', currentMonth + '/01/' + currentYear);
            //}
            $("#startDate").datepicker('setDate', currentMonth + '/01/' + currentYear);
            $("#endDate").datepicker('setDate', currentMonth + '/01/' + currentYear);
            $("#currentMonth").val(currentMonth);
        }
        else if (delta < 0) {
              var cm = new Date(currentView.end._d.getFullYear(), currentView.end._d.getMonth(), currentView.end._d.getDate() - 10);
              var currentMonth = cm.getMonth() == '0' ? '12' : cm.getMonth();
              var currentYear = cm.getMonth() == '0'  ? cm.getFullYear() - 1 : cm.getFullYear();
            //if ($("#startDate").val() != "") {
